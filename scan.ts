...
var theScanTimeout: NodeJS.Timeout | null = null;
...
function logDevice(dev: Device) {
  console.log("Scanned device '" +
    "name='" + dev.name + "' " +
    (
      (dev.localName && dev.localName != dev.name) ?
    ...
    "id='" + dev.id + "' " +
    "rssi=" + dev.rssi +
    " serviceUuids=" + JSON.stringify(dev.serviceUUIDs) + "" +
    " overflowServiceUUIDs=" + dev.overflowServiceUUIDs +
...
export function scanForDevices(maxSecondsToScan: number, scenario: ScanScenario): void {
  ...
  theBleManager.startDeviceScan(
    ...
    (error: BleError | null, scannedDevice: Device | null) => {
      ...
      let now = new Date().getTime(); // Not shared with outer scope
